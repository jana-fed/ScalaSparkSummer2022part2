...
object Day21RowsDataFrameTransformations extends App {
  ...
  val myFoodRows = Seq(Row("Banana", 3, 1.19, true),
    Row("Dumplings with meat", 10, 2.69, false),
    Row("Oat Cookies", 20, 3.09, null))

  val myFoodSchema = new StructType(Array(
    StructField("food", StringType, false), //so true refers to this field/column being nullable namely could have null
    StructField("quantity", IntegerType, false),
    StructField("price", DoubleType, false),
    StructField("isIt Vegan", BooleanType, true)))
  //TODO create a dataFrame called foodFrame which will hold those Rows

  val myRDDD = spark.sparkContext.parallelize(myFoodRows)
  val foodDF = spark.createDataFrame(myRDDD, myFoodSchema)
  println("Food dataframe")
  foodDF.show()
  //Use Select or/an SQL syntax to select and show only name and qty

  val newFoodDF = foodDF.select("food", "quantity")
  newFoodDF.show()
