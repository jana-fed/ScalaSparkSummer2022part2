...
object Day22MoreColumnTransformations extends App {
  ...
  df2.where("count > 10 AND ORIGIN_COUNTRY_NAME = 'United States'").show(5) //works but better to chain multiple filters for optimization
  ...
  val dfSample2014 = df2.sample(withReplacement, fraction2, seed)
  dfSample.show(5)
  println(s"We got ${dfSample.count()} samples")
  ...
  val df2Split = df2.randomSplit(Array(2,9,5), seed)
  getDataFrameStats(df2Split, df2).foreach(println)

  for ((dFrame, i) <- df2Split.zipWithIndex) {
    println(s"DataFrame No. $i has ${dFrame.count} rows")
  }

  val df2precentage= getDataFrameStats(df2Split, df2)
  println("DataFrame percentages")
  df2precentage.foreach(println)
